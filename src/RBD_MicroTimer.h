...
#ifndef RBD_MICROTIMER_H
#define RBD_MICROTIMER_H
...
namespace RBD {
  class MicroTimer {
    public:
      MicroTimer();                         // constructor with zero timeout, starts in "expired" state by default
      MicroTimer(unsigned long value);      // overloaded constructor: provide a setTimeout in microseconds, starts in "expired" state by default    *
      void setTimeout(unsigned long value); // set/change how long the timer will run until it expires in microseconds                               *
      void setHertz(long value);            // set/change how many times onRestart will return true in one second
      ...
      long getHertz();                      // returns the value of setHertz, which is how many times onRestart will return true in one second
    ...
    private:
      ...
      long _hertz              = 0;         // the value given to setHertz
